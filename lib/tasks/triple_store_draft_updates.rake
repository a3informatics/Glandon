namespace :triple_store do
  ...
  def display_results(items, labels, widths=[])
    ...
    results[1..-1].each { |x| puts format % x }
    puts "\n\n"
  end
  ...
  def identify_summary_changes(items)
    ...
    items.each do |x| 
      ...
      results << {
        ...
        difference: curr_children.count != curr_children.count || created.any? || deleted.any? ? "***" : ""
      }
    end
  ...
  def custom_properties_difference(current, curr_child, curr_child_cp, prev_child_cp)
    ...
    names.each do |name|
      ...
      next if curr == prev
      results << {cl: current.identifier, cli: curr_child.identifier, name: name, previous: prev[:value], current: curr[:value]}
    end
    results 
  end
  ...
  def item_difference(current, curr_child, prev_child)
    ...
    difference.each {|k,v| results << {cl: current.identifier, cli: curr_child.identifier, name: k, previous: v[:previous], current: v[:current]} if !v.is_a?(Array) && v.key?(:status) && v[:status] == :updated }
    pt = difference.dig(:preferred_term, :label)
    results << {cl: current.identifier, cli: curr_child.identifier, name: :preferred_term, previous: pt[:previous], current: pt[:current]} if !pt.nil? && pt[:status] == :updated
    difference[:synonym].each do |x|
      syn = x.dig(:label)
      results << {cl: current.identifier, cli: curr_child.identifier, name: :synonym, previous: syn[:previous], current: syn[:current]} if !syn.nil? && syn[:status] == :updated
    end
    results
  end
  ...
  def identify_detailed_changes(items)
    items.each do |x| 
      ...
      previous = Thesaurus::ManagedConcept.find_full(identify_previous(x[:s]))
  ...
  task :draft_updates => :environment do
    ...
    display_results(@changes, ["Code List", "Item", "Name", "Previous", "Current"], [0, 0, 0, 60, 60])
  end
