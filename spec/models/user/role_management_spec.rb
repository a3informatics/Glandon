...
describe "User::RoleManagememt" do
  ...
  describe "basic tests" do
    ...
    it "save_roles_and_settings" do
      expected = [Role.where_only(name: 'reader').uri.to_s, Role.where_only(name: 'term_reader').uri.to_s, Role.where_only(name: 'curator').uri.to_s]
      results = @user.role_list
      result = @user.add_role(:term_reader)
      result = @user.add_role(:curator)
      result = @user.save_roles_and_scopes
      expect(result).to eq(true)
      expect(@user.read_setting(:roles).value.split("|")).to match_array(expected)
    end

    it "save_roles_and_scopes, error" do
      ua = User::Access.where_only(user_id: @user.id)
      ua.delete
      expect{@user.save_roles_and_scopes}.to raise_error(Errors::ApplicationLogicError, 'User access node not found when saving roles and scopes.')
    end

    it "restore_roles_and_scopes" do
      result = @user.add_role(:term_reader)
      result = @user.add_role(:curator)
      @user.save_roles_and_scopes
      ua = User::Access.where_only(user_id: @user.id)
      ua.delete
      User.restore_roles_and_scopes
      results = @user.role_list
      expect(results).to match_array(["Terminology Reader", "Curator", "Reader"])
    end
