...
describe "User" do
  ...
  describe "authrosize tests" do
    ...
    it "returns allocated roles, single" do
      expect{@user.authorized?(BiomedicalConceptInstance, :create)}.to raise_error(User::NotAuthorizedError, "You are not authorized for the action.")
      expect(@user.authorized?(BiomedicalConceptInstance, :read)).to eq(true)
      expect{@user.authorized?(BiomedicalConceptInstance, :update)}.to raise_error(User::NotAuthorizedError, "You are not authorized for the action.")
      expect{@user.authorized?(BiomedicalConceptInstance, :delete)}.to raise_error(User::NotAuthorizedError, "You are not authorized for the action.")
      @user.add_role :curator
      expect(@user.authorized?(BiomedicalConceptInstance, :create)).to eq(true)
      expect(@user.authorized?(BiomedicalConceptInstance, :read)).to eq(true)
      expect(@user.authorized?(BiomedicalConceptInstance, :update)).to eq(true)
      expect(@user.authorized?(BiomedicalConceptInstance, :delete)).to eq(true)
    end

    it "authrosization checked?" do
      expect(@user.authorization_checked?).to eq(false)
      expect(@user.authorized?(BiomedicalConceptInstance, :read)).to eq(true)
      expect(@user.authorization_checked?).to eq(true)
    end
