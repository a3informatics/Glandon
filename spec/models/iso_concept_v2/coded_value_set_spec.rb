...
describe IsoConceptV2::CodedValueSet do
  ...
  describe "basic tests" do
    ...
    it "create a class, collection uris" do
      set = IsoConceptV2::CodedValueSet.new(@collection_as_uris)
      check_file_actual_expected(set.items.map{|x| x.to_h}, sub_dir, "new_expected_2.yaml", equate_method: :hash_equal)
    end

    it "create a class, collection ids" do
      set = IsoConceptV2::CodedValueSet.new(@collection_as_ids)
      check_file_actual_expected(set.items.map{|x| x.to_h}, sub_dir, "new_expected_2.yaml", equate_method: :hash_equal)
    end
    ...
    it "adds an item" do
      ...
      context_uri = Uri.new(uri: "http://www.assero.co.uk/A#context")
      set.add({id: ref_uri, context_id: context_uri})
      check_file_actual_expected(set.items.map{|x| x.to_h}, sub_dir, "new_expected_3.yaml", equate_method: :hash_equal)
    end

    it "sorts item" do
      things = []
      [{ordindal: 1, ref: "A"}, {ordindal: 2, ref: "B"}, {ordindal: 3, ref: "C"}, {ordindal: 4, ref: "D"}].each do |x|
        things << create_ref(x[:ordindal], x[:ref])
      end
      set = IsoConceptV2::CodedValueSet.new(things)
      set.sort!([{id: things[3].uri.to_id}, {id: things[2].uri.to_id}, {id: things[1].uri.to_id}, {id: things[0].uri.to_id}])
      check_file_actual_expected(set.items.map{|x| x.to_h}, sub_dir, "new_expected_4.yaml", equate_method: :hash_equal)
    end
  ...
  describe "basic tests" do
    ...
    it "update" do
      ...
      set = IsoConceptV2::CodedValueSet.new(collection)
      ...
      check_file_actual_expected(set.items.map{|x| x.to_h}, sub_dir, "update_expected_1.yaml", equate_method: :hash_equal, write_file: true)
    end
