...
describe Form::Item::Mapping do
  ...
  it "validates a valid object" do
    result = Form::Item::Mapping.new
    result.mapping = "EGMONKEY when XXTESTCD=HELLO"
    expect(result.valid?).to eq(true)
  end

  it "does not validate an invalid object, text label" do
    result = Form::Item::Mapping.new
    result.mapping = "EGMONKEY when --TESTCD=HELLO"
  ...
  it "allows an object to be found" do
    ...
    #write_hash_to_yaml_file_2(item.to_json, C_SUB_DIR, "mapping_find.yaml")
    expected = read_yaml_file_to_hash_2(C_SUB_DIR, "mapping_find.yaml")
