...
puts colourize("Load 2.6 excel ...", "blue")
    ...
    actual = read_public_yaml_file("test", filename)
  copy_file_from_public_files_rename("test", filename, sub_dir, "import_errors_expected_7a.yaml")
    check_file_actual_expected(actual, sub_dir, "import_errors_expected_7a.yaml")
    #copy_file_from_public_files("test", filename, sub_dir)
    filename = "sponsor_term_format_one_#{@object.id}_load.ttl"
  copy_file_from_public_files_rename("test", filename, sub_dir, "import_input_7a.ttl")
    expect(@job.status).to eq("Complete")
    delete_data_file(sub_dir, filename)
...
puts colourize("Load 3.0 excel ...", "blue")
    ...
    #expect(public_file_does_not_exist?("test", filename)).to eq(true)
    actual = read_public_yaml_file("test", filename)
  copy_file_from_public_files_rename("test", filename, sub_dir, "import_errors_expected_7b.yaml")
    check_file_actual_expected(actual, sub_dir, "import_errors_expected_7b.yaml")
    #copy_file_from_public_files("test", filename, sub_dir)
    filename = "sponsor_term_format_one_#{@object.id}_load.ttl"
    #expect(public_file_exists?("test", filename)).to eq(true)
    copy_file_from_public_files("test", filename, sub_dir)
  copy_file_from_public_files_rename("test", filename, sub_dir, "import_expected_7b.ttl")
