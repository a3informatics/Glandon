//= require d3_tree_rect
//= require rspec_helper

describe("D3 Tree Rect", function() {
  ...
  it("initialises the tree", function() {
 	...
 	it("allows a node to be marked by reference", function() {
 		var gRef = d3FindGRef(2);
		d3MarkNode(gRef);
		expect(selectedNodeTestRect(getFillRect(gRef))).to.equal(true);
  });
  ...
  it("allows the data from a node to be obtained by label and searched for", function() {
    ...
    expect(gRef.childNodes[1].className.animVal).to.equal('');
 	...
 	it("allows a node to be restored", function() {
		...
		data.enabled = false;
		d3RestoreNode(gRef);
		expect(disabledNodeTestRect(getFillRect(gRef))).to.equal(true);
		data.enabled = true;
		d3RestoreNode(gRef);
		expect(enabledNodeTestRect(getFillRect(gRef))).to.equal(true);
  });
