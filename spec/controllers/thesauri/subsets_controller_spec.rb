...
describe Thesauri::SubsetsController do
  ...
  describe "Authorized User" do
    ...
    it "remove" do
      request.env['HTTP_ACCEPT'] = "application/json"
      subset = Thesaurus::Subset.find(Uri.new(uri: "http://www.assero.co.uk/TS#54176c59-b800-43f5-99c3-d129cb563c79"))
      sm_uri = Uri.new(uri:"http://www.assero.co.uk/TSM#67871de3-5e13-42da-9814-e9fc3ce7bddd")
      #expect(Thesaurus::Subset).to receive(:find).and_return(Thesaurus::Subset.new)
    ...
    it "move after" do
      ...
      after_uri = Uri.new(uri:"http://www.assero.co.uk/TSM#67871de3-5e13-42da-9814-e9fc3ce7baaa")
      #expect(Thesaurus::Subset).to receive(:find).and_return(Thesaurus::Subset.new)
      put :move_after, {id: subset.uri.to_id, subset:{member_id: member_uri.to_id, after_id: after_uri.to_id }}
      expect(response.content_type).to eq("application/json")
      expect(response.code).to eq("200")
    end
