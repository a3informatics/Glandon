...
describe Forms::Items::TextLabelsController do
  ...
  describe "Move down" do
    ...
    it "Move down I" do
      ...
      token = Token.obtain(@form, @user)
      audit_count = AuditTrail.count
      put :move_down, params:{id: item.id, text_label: {parent_id: parent.id , form_id: @form.id}}
      expect(AuditTrail.count).to eq(audit_count+1)
      actual = check_good_json_response(response)
    end
    ...
    it 'Move down I, Error locked by another user' do
      ...
      expect(AuditTrail.count).to eq(audit_count)
      actual = check_error_json_response(response)
      check_file_actual_expected(actual.to_h, sub_dir, "move_down_text_label_error_expected_1.yaml", equate_method: :hash_equal)
    end
