  ...
  this.editorTable = $('#editor_table').DataTable({
    ...
    columns: [
      ...
      { data: null, "render": function(data,type,full,meta) {
        ...
        return '<button type="button" class="btn medium"' + disabled + '>Edit</button>';
      }},
      { data: null, "render": function(data,type,full,meta) {
        return '<button type="button" class="btn medium red">Delete</button>';
      }}
    ],
    rowId: "id",
    keys: {
  ...
  $('#editor_table').on('click', 'tbody td:not(:first-child)', function (e) {
    ...
    if (col == 5) {
      if (data.edit_path !== "") {
        ...
        //keepToken = true; // Keep the token. Important @todo need to consider what to do here for propogating the token
