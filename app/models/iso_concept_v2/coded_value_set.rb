...
class IsoConceptV2
  ...
  class CodedValueSet
    ...
    def update(params)
      ...
      @items = []
      params[:has_coded_value].each_with_index {|x,i| add(x, i+1, params[:transaction])}
      update_items(params[:transaction])
    end

    def add(params, ordinal, tx=nil)
      ref = OperationalReferenceV3::TucReference.new(context: Fuseki::Base.as_uri(params[:context_id]), reference: Fuseki::Base.as_uri(params[:id]), optional: true, ordinal: ordinal, transaction: tx)
