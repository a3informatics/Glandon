...
class Form
  ...
  module CRF
    ...
    def get_css
      html = "<style>"
      html += "table.crf-input-field tr td { border-right: 1px dashed; border-left: 1px dashed; border-bottom: 1px solid;}\n"
      html += "table.crf-input-field tr td { font-family: Arial, \"Helvetica Neue\", Helvetica, sans-serif; font-size: 9pt; text-align: center; "
      html += "vertical-align: center; padding: 5px 10px; }\n"
      html += "table.crf-input-field td:last-child {border-right: 1px solid}\n"
      html += "table.crf-input-field td:first-child {border-left: 1px solid}\n"
    ...
    def field_table(cell_content)
      ...
      cell_content.each do |cell|
        if separators.include? cell
          html += "<td class=\"no-border\">#{cell}</td>"
        else
          html += "<td>#{cell}</td>"
        end
      end
