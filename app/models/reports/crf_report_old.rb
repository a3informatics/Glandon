class Reports::CrfReport < Reports::PdfReport
  ...
  def initialize(node, options, annotations, user)
    ...
    if ci_nodes.length > 0
      ...
      ConsoleLogger.log(C_CLASS_NAME, "Initialize", "CI Date=" + data.to_s)
      start_new_page(:layout => :portrait)
...
private
  ...
  def crf_node(node, options, annotations, ci_nodes, note_nodes)
    ...
    if node[:type] == "Form"
      ...
      if node[:repeating]
        ...
        if options[:annotate]
          node[:children].each do |child|
            row << "<color rgb='FF0000'>#{child[:mapping]}</color>"
          end
          data << row
        end
        row = Array.new
        node[:children].each do |child|
          row << input_field(child)
        end
        data << row
        my_table = make_table(data, :cell_style => { :inline_format => true}) do
          cells.padding = 5
        end
        rows << [{:colspan => 3, :content => my_table}]
      else
        node[:children].each do |child|
          rows += crf_node(child, options, annotations, ci_nodes, note_nodes)
        end
  ...
  def format_nodes(nodes, symbols)
    ...
    nodes.each_with_index do |node, index|
      ...
      #html = MarkdownEngine::render(text)
      #html = ActionController::Base.helpers.sanitize html, tags: %w(b h1 h2 h3 h4 li ul ol small i u)
      text = "<b>Bold</b><i>Heading 1</i><br/><small>List 1</small>"
