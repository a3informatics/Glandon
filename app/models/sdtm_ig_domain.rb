  ...
  def get_children
    ...
    query_string = %Q{
      ...
      {         
        ...
        ?c bd:format ?format .
        OPTIONAL {?c bd:ctReference/bo:reference ?ct_reference .}
    ...
    query_results.by_object_set([:ordinal, :c, :type, :label, :name, :ct_and_format, :format, :ct_reference, :description, :used, :compliance, :compliance_label, :typed_as, :typed_as_label, :classification, :classification_label, :standard]).each do |x|
      ...
      ct_and_format: x[:ct_and_format], format: x[:format], ct_reference: x[:ct_reference].to_s, description: x[:description], used: x[:used].to_bool, compliance: {id: x[:compliance].to_id, label: x[:compliance_label]}, typed_as: {id: x[:typed_as].to_id, label:x[:typed_as_label]}, classified_as: {id:x[:classification].to_id, label: x[:classification_label]} }
    end
    results
  end
