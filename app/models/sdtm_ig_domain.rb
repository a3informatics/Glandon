class SdtmIgDomain < Tabulation
  ...
  def get_children
    results = []
    query_string = %Q{
      SELECT DISTINCT ?ordinal ?c ?type ?label ?name ?ct_and_format ?format ?ct_reference ?description ?notes ?comment ?method ?used ?compliance ?compliance_label ?typed_as ?typed_as_label ?classification ?classification_label ?standard WHERE       
      {         
        ...
        ?c bd:format ?format .
        OPTIONAL {?c bd:method ?method .}
        OPTIONAL {?c bd:comment ?comment .}
        OPTIONAL {?c bd:notes ?notes .}
    ...
    query_results.by_object_set([:ordinal, :c, :type, :label, :name, :ct_and_format, :format, :ct_reference, :description, :notes, :comment, :method, :used, :compliance, :compliance_label, :typed_as, :typed_as_label, :classification, :classification_label, :standard]).each do |x|
      ...
      ct_and_format: x[:ct_and_format], format: x[:format], ct_reference: set_ct_reference(x[:ct_reference]), description: x[:description], used: x[:used].to_bool, compliance: {id: x[:compliance].to_id, label: x[:compliance_label]}, 
      typed_as: {id: x[:typed_as].to_id, label:x[:typed_as_label]}, classified_as: {id:x[:classification].to_id, label: x[:classification_label]}, notes: x[:notes], method: x[:method], comment: x[:comment] }
    end
    results
  end
  ...
  def unique_name_in_domain?(variable, name)
