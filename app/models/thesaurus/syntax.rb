class Thesaurus
  ...
  class Syntax
    ...
    def array_to_sparql(column)
      ...
      when 1
        @parts.first.first == "\"" && @parts.first.last == "\"" ? type = :EXACT : type = :CONTAINS #EXACT or CONTAINS operator
      end
  ...
  private
    def type_to_sparql(type, results, column)
      ...
      when :CONTAINS
        return contains_match_statement(results, column)
      else
    ...
    def exact_match_statement(elements, column)
     return " FILTER (#{column} = '#{elements[0].gsub("\"", "")}') . \n "
    end

    # Get the query string as part of
    def contains_match_statement(elements, column)
