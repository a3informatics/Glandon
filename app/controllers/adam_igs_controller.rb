...
class AdamIgsController < ApplicationController
  ...
  def export_ttl
    ...
    send_data to_turtle(@adam_ig.triples), filename: "#{@adam_ig.owner}_#{@adam_ig.identifier}.ttl", type: 'application/x-turtle', disposition: 'inline'
  end
  
  def export_json
    @adam_ig = AdamIg.find(params[:id], the_params[:namespace])
    send_data @adam_ig.to_json.to_json, filename: "#{@adam_ig.owner}_#{@adam_ig.identifier}.json", :type => 'application/json; header=present', disposition: "attachment"
  end
