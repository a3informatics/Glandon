...
class SdtmUserDomainsController < ManagedItemsController
  ...
  def show
    @sdtm_user_domain = SdtmUserDomain.find_minimum(protect_from_bad_id(params))
    @show_path = show_data_sdtm_user_domain_path(@sdtm_user_domain)
    @close_path = history_sdtm_user_domains_path(:sdtm_user_domain => {identifier: @sdtm_user_domain.has_identifier.identifier, scope_id: @sdtm_user_domain.scope})
  end

  def show_data
    @sdtm_user_domain = SdtmUserDomain.find_minimum(protect_from_bad_id(params))
    items = @sdtm_user_domain.get_items
...
private
  ...
  def path_for(action, object)
    case action
      when :show
        return sdtm_user_domain_path(object)
      when :edit
        return ""
      else
        return ""
  ...
  def history_path_for(identifier, scope_id)
    return {history_path: history_sdtm_user_domains_path({sdtm_user_domain:{identifier: identifier, scope_id: scope_id}})} 
  end

  def close_path_for
    sdtm_user_domains_path
  end           
