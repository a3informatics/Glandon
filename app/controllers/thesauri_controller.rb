...
class ThesauriController < ApplicationController
  ...
  def create
    ...
    if @thesaurus.errors.empty?
      AuditTrail.create_item_event(current_user, @thesaurus, audit_message(:created))
  ...
  def clone
    ...
    if th.errors.empty?
      AuditTrail.create_item_event(current_user, @thesaurus, audit_message(:cloned))
  ...
  def add_child
    ...
    if !token.nil?
      ...
      if tc.errors.empty?
        AuditTrail.update_item_event(current_user, ct, audit_message(:updated)) if token.refresh == 1
  ...
  def destroy
    ...
    if !token.nil?
      thesaurus.delete
      AuditTrail.delete_item_event(current_user, thesaurus, audit_message(:deleted))
      token.release
    else
  ...
  def extension
    ...
    if !th.nil?
      ...
      AuditTrail.create_item_event(current_user, th, audit_message(:updated))
      AuditTrail.create_item_event(current_user, new_object, audit_message(:created, "extension"))
  ...
  def add_subset
    ...
    AuditTrail.create_item_event(current_user, new_mc, audit_message(:updated, "subset"))
